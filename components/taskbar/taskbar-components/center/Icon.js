...
export default function Icon({ name, title }) {
  return (
    <div className="relative flex items-center justify-center group">
      ...
      {/* Tooltip */}
      <div className="pointer-events-none absolute -top-9 left-1/2 -translate-x-1/2 bg-zinc-800/95 backdrop-blur-sm border border-white/10 px-2.5 py-1 rounded text-xs whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity duration-150 shadow-lg">
        {title}
      </div>
    </div>
  );
